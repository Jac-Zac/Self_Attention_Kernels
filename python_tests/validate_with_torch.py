...
- Runs cmhsa.out in validation mode to produce raw float32 files in python_test/
...
- Cleans python_test/ artifacts on success
"""
...
def run_bin(bin_path: str, B: int, H: int, S: int, D: int, seed: int):
    cmd = [
        bin_path,
        "--validate-outdir",
        "python_test",
...
def main():
    ...
    outdir = Path("python_test")
    outdir.mkdir(parents=True, exist_ok=True)

    # Run C binary to generate artifacts
    run_bin(args.bin, args.batch, args.n_heads, args.seq_len, args.head_dim, args.seed)
    ...
    # Clean artifacts
    for fname in ["q.bin", "k.bin", "v.bin", "out.bin", "meta.json"]:
        try:
            (outdir / fname).unlink()
        except FileNotFoundError:
            pass

    print("Validation PASS; cleaned artifacts")
