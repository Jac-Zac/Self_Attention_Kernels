...
cmhsa_forward_kernel(const float *RESTRICT Q, const float *RESTRICT K,
  ...
  // ===========================================================================
  // Step 1: Compute scaled dot-product scores with warp-parallel dot products
  // ===========================================================================
  ...
  for (size_t key_pos = 0; key_pos <= q; key_pos++) {
    ...
    // Warp-parallel dot product: each thread computes partial sum
    // Thread i handles dimensions: i, i+32, i+64, ...
    float dot_partial = 0.0f;
    for (size_t d = lane_id; d < head_dim; d += warpSize) {
      dot_partial += Q[query_offset + d] * K[key_offset + d];
    }

    // Reduce partial sums across warp (result in lane 0)
    float dot_product = warp_reduce_sum(dot_partial);
    float score = dot_product * scale;

    // Lane 0 updates max and stores score
    if (lane_id == 0) {
  ...
  // ===========================================================================
  // Step 2: Numerically stable softmax (only lane 0 computes)
  // ===========================================================================
  float sum_exp = 0.0f;
  if (lane_id == 0) {
    for (size_t key_pos = 0; key_pos <= q; key_pos++) {
  ...
  // ===========================================================================
  // Step 3: Weighted sum of values (warp-parallel across head_dim)
  // ===========================================================================
