...
// ============================================================================
// v2_smem: Shared Memory
// ============================================================================
// Add optimizations from v2 to leverage shared memory:
//
// 1. Shared Memory (v2_smem):
...
cmhsa_forward_kernel(const float *RESTRICT Q, const float *RESTRICT K,
  ...
  for (size_t d = lane_id; d < head_dim; d += WARP_SIZE) {
    float acc = 0.0f;
    // We iterate through keys; for each key, we read the weight and V[d]
    for (int key_pos = 0; key_pos <= q; key_pos++) {
      acc +=
          aw[key_pos] * inv_sum_exp * V[bh_offset + key_pos * head_dim_pad + d];
    }
    out[output_offset + d] = acc;
  }
}
...
static CudaConfig make_cuda_config(const AttentionDims dims) {
  ...
  // Shared memory size: 1 Query per warp
