...
// ============================================================================
// vx_flash: QK Tiling with Collaborative Loading
// ============================================================================
// CORRECTED VERSION: All threads load K collaboratively (not serial)
//
// Key Optimizations:
//   - TILE_K=8: Minimal shared memory (4 KB)
//   - Collaborative loading: All threads load K in parallel
//   - Bank-conflict-free access
//   - Q in registers: Loaded once, reused
//   - V with __ldg__(): Read-only cache hints
...
cmhsa_forward_kernel(const float *RESTRICT Q, const float *RESTRICT K,
  ...
  float *RESTRICT smem_K = smem;

  float r_Q[4] = {0}, r_K[4], r_V[4], r_out[4] = {0};

  if (active) {
    const size_t q_global_offset = bh_offset + q_idx * head_dim_pad + d_base;
    float4 q_vec = *reinterpret_cast<const float4 *>(&Q[q_global_offset]);
    r_Q[0] = q_vec.x;
    r_Q[1] = q_vec.y;
    r_Q[2] = q_vec.z;
    r_Q[3] = q_vec.w;
  ...
  for (int tile_start = 0; tile_start <= q_idx; tile_start += TILE_K) {
    int tile_end = min(tile_start + TILE_K, q_idx + 1);

    if (active) {
      for (int k = tile_start; k < tile_end; k++) {
        int k_idx = k - tile_start;
        const float *RESTRICT k_row = &K[bh_offset + k * head_dim_pad + d_base];

        float4 k_vec = *reinterpret_cast<const float4 *>(k_row);
        smem_K[k_idx * head_dim_pad + d_base] = k_vec.x;
        smem_K[k_idx * head_dim_pad + d_base + 1] = k_vec.y;
        smem_K[k_idx * head_dim_pad + d_base + 2] = k_vec.z;
        smem_K[k_idx * head_dim_pad + d_base + 3] = k_vec.w;
      }
    }
    ...
    for (int k = tile_start; k < tile_end; k++) {
      ...
      if (active) {
        float4 k_vec =
            *reinterpret_cast<float4 *>(&smem_K[k_idx * head_dim_pad + d_base]);
        r_K[0] = k_vec.x;
        r_K[1] = k_vec.y;
        r_K[2] = k_vec.z;
        r_K[3] = k_vec.w;
...
__host__ void cmhsa_forward_cuda(const float *RESTRICT Q,
  ...
  size_t shared_mem_size = TILE_K * dims.head_dim_padded * sizeof(float);

  cmhsa_forward_kernel<<<grid, block, shared_mem_size>>>(Q, K, V, out, dims,
