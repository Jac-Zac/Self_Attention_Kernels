...
// ============================================================================
// v5: Register Blocking + float4 Vectorized Loads
// ============================================================================
// Changes from v4:
// - Q loaded into registers ONCE, reused for all keys
// - float4 loads: single 128-bit transaction for 4 floats
// - Contiguous access: lane i handles elements [4*i, 4*i+3]
// ============================================================================
...
cmhsa_forward_kernel(const float *RESTRICT Q, const float *RESTRICT K,
  ...
  // Contiguous access: lane i handles [4*i, 4*i+3]
  ...
  for (int k_idx = 0; k_idx <= q_idx; k_idx++) {
    ...
    // Load K with float4
    if (active) {
      float4 k_vec = *reinterpret_cast<const float4 *>(&K[kv_base]);
      r_K[0] = k_vec.x;
      r_K[1] = k_vec.y;
      r_K[2] = k_vec.z;
      r_K[3] = k_vec.w;
    ...
    // Load V with float4
    if (active) {
      float4 v_vec = *reinterpret_cast<const float4 *>(&V[kv_base]);
      r_V[0] = v_vec.x;
      r_V[1] = v_vec.y;
      r_V[2] = v_vec.z;
      r_V[3] = v_vec.w;
