...
cmhsa_forward_kernel(const float *RESTRICT Q, const float *RESTRICT K,
  ...
  for (size_t d = lane_id; d < head_dim; d += warpSize * 4) {
    float acc0 = 0.0f;
    float acc1 = 0.0f;
    float acc2 = 0.0f;
    float acc3 = 0.0f;
    ...
    for (int k = 0; k <= q; ++k) {
      ...
      const float *v_ptr = V + bh_offset + k * head_dim_pad;

      acc0 += w * v_ptr[d];
      if (d + warpSize < head_dim)
        acc1 += w * v_ptr[d + warpSize];
      if (d + 2 * warpSize < head_dim)
        acc2 += w * v_ptr[d + 2 * warpSize];
      if (d + 3 * warpSize < head_dim)
        acc3 += w * v_ptr[d + 3 * warpSize];
    }

    // write back
    out[output_offset + d] = acc0;
    if (d + warpSize < head_dim)
      out[output_offset + d + warpSize] = acc1;
    if (d + 2 * warpSize < head_dim)
      out[output_offset + d + 2 * warpSize] = acc2;
    if (d + 3 * warpSize < head_dim)
      out[output_offset + d + 3 * warpSize] = acc3;
  }

  /*
  for (size_t d = lane_id; d < head_dim; d += warpSize * 2) {
    float acc0 = 0.0f;
    float acc1 = 0.0f;

    for (int key_pos = 0; key_pos <= q; key_pos++) {
      const size_t value_offset = bh_offset + key_pos * head_dim_pad;
      float w = aw[key_pos] * inv_sum_exp;

      acc0 += w * V[value_offset + d];
      if (d + warpSize < head_dim)
        acc1 += w * V[value_offset + d + warpSize];
    }
