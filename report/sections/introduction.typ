...
For benchmarking, I report speedups against two PyTorch baselines:

- *PyTorch naive:* a straightforward attention implementation expressed as standard PyTorch tensor operations (matrix multiplications, explicit causal masking, softmax, and a final matrix multiplication). This is representative of what one might write directly in PyTorch, and while each operator is highly optimized, the overall computation is not fused into a single attention kernel.

- *PyTorch SDPA:* PyTorch's `scaled_dot_product_attention` in causal mode (`is_causal=true`) and default settings. Depending on device, build, and tensor shapes, PyTorch may dispatch to a highly optimized fused kernel.

== Hardware Platform

CPU benchmarks in this work are conducted on the Orfeo HPC cluster, using two different partitions depending on the experiment:

- *Single-threaded CPU (GENOA partition):* 13 AMD nodes, each equipped with two #link("https://www.amd.com/en/products/processors/server/epyc/4th-generation-9004-and-8004-series/amd-epyc-9374f.html")[AMD EPYC 9374F] processors and 512 GB of RAM.
...
- *Multi-threaded CPU (EPYC partition):* 8 nodes equipped with two #link("https://www.amd.com/en/products/processors/server/epyc/7002-series/amd-epyc-7h12.html")[AMD EPYC 7H12] processors. Used for strong scaling experiments with OpenMP.

  Key specifications:
  - *Architecture:* x86_64 (Zen 2)
  - *Cores:* 128 per node (64 per socket $times$ 2 sockets, SMT disabled)
  - *NUMA nodes:* 8
